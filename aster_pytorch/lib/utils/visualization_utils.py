...
def recognition_vis(images, preds, targets, scores, dataset, vis_dir, output_score, paths):
  ...
  for id, (image, pred, target, score, score_list, path) in enumerate(zip(images, pred_list, targ_list, scores, output_score, paths)):
    if pred.lower() == target.lower():
      flag = 'right'
    else:
      flag = 'error'
    file_name = '{:}_{:}_{:}_{:}_{:.3f}.jpg'.format(flag, id, pred, target, score)
    # max_len = images.size(1)
    
    # print(score_list[:len(pred) ])
    # if flag == 'error':
    f.write(str(path) +'\n')
    f.write(str(pred) + '\n')
    f.write(str(target) + '\n')
    f.write(str(score_list[:len(pred)].cpu().numpy())+'\n \n')
