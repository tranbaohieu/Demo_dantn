...
from .transformer import TransformerDecoderLayer, TransformerDecoder, Transformer
import math 
...
class AttentionRecognitionHead(nn.Module):
  ...
  """
  def __init__(self, num_classes, in_planes, sDim, attDim, max_len_labels, encoder_block, decoder_block):
    ...
    self.sDim = sDim
    self.attDim = attDim
    self.max_len_labels = max_len_labels + 1
    self.target_mask = None
    self.decoder = DecoderUnit(sDim=sDim, xDim=in_planes, yDim=num_classes, attDim=attDim, num_encoders= encoder_block, num_decoders= decoder_block)
    # self.num_encoders = encoder_block
    # self.num_decoders = decoder_block
  ...
  def forward(self, x):
    ...
    batch_targets = torch.zeros(batch_size, max_lens)
    # print('batch tgt: ', batch_targets.size())
    batch_targets[:, 0] = torch.zeros((batch_size)).fill_(self.num_classes)
    batch_targets[:, 1:] = targets[:, :(max_lens-1)]
    # Decoder
    state = torch.zeros(1, batch_size, self.sDim)
    # outputs = []
    outputs = self.decoder(x, state, batch_targets)
    return outputs
  ...
  def sample(self, x):
    ...
    outputs = torch.zeros(batch_size, self.max_len_labels)
    outputs[:, 0] = torch.zeros((batch_size)).fill_(self.num_classes)
    for i in range(1, self.max_len_labels):
      output = self.decoder(x, state, outputs[:, :i])
      output = output[:, -1, :]
  ...
  def beam_search(self, x, beam_width, eos):
    ...
    # print(inflated_encoder_feats .size())
    ...
    outputs = torch.zeros(batch_size * beam_width, self.max_len_labels)
    outputs[:, 0] = torch.zeros((batch_size * beam_width)).fill_(self.num_classes)

    for i in range(1, self.max_len_labels):
      output = self.decoder(inflated_encoder_feats, state, outputs[:, :i])
      output = output[:, -1, :]
      ...
      output = (candidates % self.num_classes).view(batch_size * beam_width)
      # print(output.size())
      outputs[:, i] = output
      ...
      eos_indices = output.view(-1, 1).eq(eos)
      if eos_indices.nonzero().dim() > 0:
        sequence_scores.masked_fill_(eos_indices, -float('inf'))

      # Cache results for backtracking
      stored_predecessors.append(predecessors)
      stored_emitted_symbols.append(output)
...
class DecoderUnit(nn.Module):
  def __init__(self, sDim, xDim, yDim, attDim, num_encoders, num_decoders):
    ...
    # self.decoder_layer = TransformerDecoderLayer(d_model=512, nhead=8, dim_feedforward=2048)
    # self.decoder = TransformerDecoder(self.decoder_layer, num_layers=6)
    # self.decoder_layer = TransformerDecoderLayer(d_model=self.sDim, nhead=4, dim_feedforward=256)
    # self.transformer_decoder = TransformerDecoder(self.decoder_layer, num_layers=4)
    self.decoder = Transformer(d_model=self.sDim, nhead=4, num_encoder_layers=num_encoders,num_decoder_layers=num_decoders, dim_feedforward=256)
    self.pos = PositionalEncoding(self.emdDim, 0.2)
    self.init_weights()
  ...
  def forward(self, x, sPrev, yPrev):
    ...
    self.target_mask = self._generate_square_subsequent_mask(yPrev.size()[1]) 
    self.src_mask  = self._generate_square_subsequent_mask(x.size()[1])
    
    self.memory_mask = self._generate_square_subsequent_mask(x.size()[1])
    ...
    x = x.transpose(1,0)
    # print(x.size())]
    # output = self.transformer_decoder(yProj, x)
    output = self.decoder(x, yProj, self.src_mask, self.target_mask)
