...
class ModelBuilder(nn.Module):
  ...
  """
  def __init__(self, rec_num_classes, sDim, attDim, max_len_labels, eos, STN_ON=False, encoder_block=4, decoder_block=4):
    ...
    self.ss = [[2, 2], [2, 2], [2, 1], [2, 1], [1, 1]]
    self.ks = [[2, 2], [2, 2], [2, 1], [2, 1], [1, 1]]
    self.vgg_encoder = vgg11_bn(self.ks,self.ss, 128)
    self.resnet50 = resnet_fpn('resnet101')
    self.encoder = ResNet_ASTER(with_lstm=False)
  ...
  def forward(self, input_dict):
    ...
    # encoder_feats = self.vgg_encoder(x)
    # encoder_feats = self.resnet50(x)
    # encoder_feats = encoder_feats.transpose(1,0)
