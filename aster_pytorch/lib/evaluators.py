...
class BaseEvaluator(object):
  ...
  def evaluate(self, data_loader, step=1, print_freq=1, tfLogger=None, dataset=None, vis_dir=None):
    ...
    # forward the network
    images, outputs, targets, losses, paths = [], {}, [], [], []
    file_names = []
    ...
    #   # print(subimages)
    #   subimages = subimages.permute(1,2,0)
    #   subimages = subimages.cpu().numpy()
    #   images_cat += list(subimages)
    

    if vis_dir is not None:
      # recognition_vis(images_cat, outputs['pred_rec'], targets, score_list, dataset, vis_dir, outputs['pred_rec_score'])
      # for i, subimgs in enumerate(images):
      recognition_vis(images, outputs['pred_rec'], targets, score_list, dataset, vis_dir, outputs['pred_rec_score'], paths)
...
class Evaluator(BaseEvaluator):
  def _parse_data(self, inputs):
    ...
    if global_args.evaluate_with_lexicon:
      ...
      imgs, label_encs, lengths, paths = inputs
